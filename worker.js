// =================================================================================
// R2-UI-WORKER v3.2 (Modified by Gemini)
// Features: Light/Dark Mode, Image Previews, Lightbox, Grid/List View, Mobile-First.
// Changelog:
// - Fixed single item checkbox selection issue.
// - Grid view action menu now appears outside the item card, matching list view behavior.
// - Restored the "Move" option in the action menu.
// =================================================================================
...
<head>
  ...
  <style>
    ...
    /* MODIFIED: 移除 overflow: hidden 以便菜单弹出 */
    ...
    .grid-view .checkbox { position: absolute; top: 5px; left: 5px; z-index: 5; }
    ...
    .rename-dialog { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: var(--card-bg); padding: 20px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.2); z-index: 2000; display: none; width: 90%; max-width: 400px; }
    .rename-dialog.show { display: block; }
    .rename-dialog input { width: 100%; padding: 10px; margin-bottom: 15px; border: 1px solid var(--border-color); border-radius: 4px; background-color: var(--bg-color); color: var(--text-color); }
    .rename-dialog-buttons { display: flex; justify-content: flex-end; gap: 10px; }
...
<body>
  ...
  <div class="rename-dialog" id="rename-dialog">
    <h3>重命名文件</h3><input type="text" id="new-filename" placeholder="新文件名"><div class="rename-dialog-buttons"><button id="rename-cancel">取消</button><button id="rename-confirm">确认</button></div>
  </div>
  ...
  <div id="app-view" class="hidden">
    <header><h1>文件列表</h1><div class="actions"><button id="view-toggle-button" title="切换视图"></button><button id="select-all-button">全选</button><button id="delete-button">删除选中</button></div></header>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderFiles = () => {
    ...
    G.files.forEach(file => {
      const fileType = getFileIcon(file.key); const isImage = fileType === 'image'; const item = document.createElement('div');
      item.className = \`file-item \${G.isAllSelected ? 'selected' : ''}\`; item.dataset.key = file.key;
      const iconHTML = isImage && G.viewMode === 'grid' ? \`<img src="/\${encodeURIComponent(file.key)}" alt="\${file.key}" loading="lazy">\` : \`<svg><use xlink:href="\${isImage ? '#icon-file' : fileType}"></use></svg>\`;
      // MODIFIED: 恢复“移动”选项
      const actionsHTML = \`
      ...
      item.innerHTML = (isImage && G.viewMode === 'grid') ? \`
        ...
        <div class="icon"><svg><use xlink:href="\${fileType}"></use></svg></div>
        <div class="info"><div class="filename" title="\${file.key}">\${file.key}</div><div class="filesize">\${formatBytes(file.size)}</div></div>
        \${actionsHTML}\`;
  ...
  const handleFileAction = (action, key) => {
    ...
    switch(action) {
      case 'rename': G.newFilename.value = key; G.renameDialog.classList.add('show'); break;
      ...
      // MODIFIED: 恢复“移动”选项的提示
      case 'move': showToast('移动功能正在开发中'); break;
      ...
      case 'delete': if (confirm(\`确定删除文件 "\${key}" 吗？\`)) { handleDelete([key]); } break;
    }
  };
  
  const renameFile = async () => {
    const oldKey = G.currentFileKey, newKey = G.newFilename.value;
    if (!newKey || newKey === oldKey) { G.renameDialog.classList.remove('show'); return; }
    try { await apiCall('/api/rename', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ oldKey, newKey }) }); showToast(\`文件已重命名为 "\${newKey}"\`); await refreshFileList(); }
    catch (error) { showToast(\`重命名失败: \${error.message}\`); }
  ...
  const refreshFileList = async () => {
    try { const response = await apiCall('/api/list'); G.files = (await response.json()).sort((a,b) => new Date(b.uploaded) - new Date(a.uploaded)); G.isAllSelected = false; G.selectAllButton.textContent = '全选'; renderFiles(); }
  ...
  const handleDelete = async (keys) => {
    ...
    try { await apiCall('/api/delete', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ keys }) }); showToast(\`成功删除 \${keys.length} 个文件\`); await refreshFileList(); }
  ...
  const init = () => {
    ...
    G.fileContainer.addEventListener('click', e => {
      ...
      // 如果直接点击勾选框，则不执行这里的逻辑，交由 'change' 事件处理
      ...
      if (e.target.closest('.icon') || e.target.tagName === 'IMG') {
        if (getFileIcon(key) === 'image') { const imageIndex = G.imageFiles.findIndex(f => f.key === key); if (imageIndex > -1) openLightbox(imageIndex); }
        else { window.open(\`/\${encodeURIComponent(key)}\`, '_blank'); }
      } else if (checkbox && !e.target.closest('.menu-button, .file-actions, a')) {
    ...
    G.newFilename.addEventListener('keypress', e => e.key === 'Enter' && renameFile());
